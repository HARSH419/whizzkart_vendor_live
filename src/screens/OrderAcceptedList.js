...
class OrderAcceptedList extends React.Component {
  constructor(props) {
    ...
    this.state = {
      isLoding: true,
      TestImage:
        'https://www.pngfind.com/pngs/m/300-3005563_free-png-chicken-fried-rice-plate-png-png.png',
    };
  }
        ...
        {this.state.isLoding ? (
          ...
          <FlatList
            ...
            renderItem={item => {
              ...
              const time = d.split(' ');
              const fullAdd = ` ${
                item.item.location?.house_no ? '' : item.item.location?.house_no
              } ${item.item.location?.area} ${item.item.location?.landmark} ${
              ...
              return (
                <TouchableOpacity
                  ...
                  <OrderlistAR
                    ...
                    Name={
                      item.item.order_details[0]?.vendor_product
                        ?.product_details?.category?.name
                    }
                    image={
                      !item.item.order_details[0]?.vendor_product
                        ?.product_details?.primaryimages?.imagePath
