...
export const GetALLProductList = () => {
  return async dispatch => {
    console.log('enter GetALLProductList');
...
export const UpdateProductStatus = (
  ...
  return async () => {
    try {
      ...
      const response = await api.post('/api/updateVendorProductStatus', {
        product_id,
        status,
      });
...
export const UpdateProfileStatus = (status, cb) => {
  return async () => {
    try {
      console.log({status});
      const response = await api.post('/api/vendorActiveOrInActive', {status});
      console.log({response: response?.data});
      if (response?.data?.status == 200) {
        cb();
      }
  ...
  return async dispatch => {
    ...
    if (response.data.code == 200) {
      ...
      if (response.data.errors.email && response.data.errors.mobile) {
        callback(
          `${response.data?.errors?.email[0]} ${response.data?.errors?.mobile[0]} `,
        );
        console.log('test');
      } else if (response.data?.errors?.email && !response.data.errors.mobile) {
        callback(`${response.data?.errors?.email[0]} `);
      } else if (
        response.data?.errors?.mobile &&
        !response.data?.errors?.email
      ) {
        callback(`${response.data?.errors?.mobile[0]} `);
      }
    }
  };
};
...
export const GoogleMapsSearch = (keyword, callback) => {
  return async dispatch => {
    const response = await axios.post(
      `https://maps.googleapis.com/maps/api/place/autocomplete/json?key=${GOOGLE_SEARCH_API_MAPS}&input=${keyword}&components=country:IN`,
    );
...
export const GoogleRevGeocode = (lng, lat, callback) => {
  return async dispatch => {
    const response = await axios.post(
      `https://maps.googleapis.com/maps/api/geocode/json?latlng=${lat},${lng}&location_type=GEOMETRIC_CENTER&key=${GOOGLE_SEARCH_API_MAPS}`,
    );
...
export const GooglePlaceCode = (keyword, callback) => {
  return async dispatch => {
    const response = await axios.post(
      `https://maps.googleapis.com/maps/api/place/details/json?placeid=${keyword}&key=${GOOGLE_SEARCH_API_MAPS}`,
    );
